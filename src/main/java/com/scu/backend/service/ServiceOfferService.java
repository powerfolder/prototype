...
public class ServiceOfferService {
	...
	public String createServiceOffer(ServiceOffer argServiceOffer) throws Exception {		
		SCUMarketplace myMarketplace = SCUMarketplace.load(marketplaceContractAddress, web3j, credentials, gasPrice,
				gasLimit);
		TransactionReceipt transactionReceipt = myMarketplace.createServiceOffer(argServiceOffer.getId(), argServiceOffer.getServiceProvider(),
				argServiceOffer.getPricePerMonth(), argServiceOffer.getDescription(), argServiceOffer.getLocation())
				.send();
		List<SCUServiceOfferCreatedEventResponse> responseList = myMarketplace.getSCUServiceOfferCreatedEvents(transactionReceipt);
